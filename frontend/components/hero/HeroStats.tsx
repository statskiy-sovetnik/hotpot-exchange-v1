...
const HeroStats: FC<{ stats: Props }> = ({ stats }) => {
  return (
    <div className="grid grid-cols-3 items-center gap-2 overflow-hidden rounded-lg dark:bg-black md:m-0 md:h-[82px] md:gap-20 md:bg-white dark:md:bg-black">
      <Stat name="Items">
        <h3 className="text-xs font-semibold dark:text-white md:text-base">
          {formatNumber(stats.count)}
        </h3>
      </Stat>
      <Stat name="Floor price">
        <h3 className="flex items-center justify-center gap-1 text-xs dark:text-white md:text-base">
...
const Stat: FC<{ name: string; children: ReactNode }> = ({
  ...
  <div className="flex flex-col items-center justify-center bg-white dark:bg-black md:h-auto">
    <p className="mb-1 text-xs font-normal text-[#A3A3A3] md:text-sm">{name}</p>
    <div className="mt-1 text-xs font-medium md:text-base">{children}</div>
  </div>
)
