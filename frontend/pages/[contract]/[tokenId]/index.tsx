...
const Index: NextPage<Props> = ({
  ...
  const hotpotListingsUrl = `${HOTPOT_API}/order/collection/${router.query?.contract?.toString()}/token/${router.query?.tokenId?.toString()}?chain=mainnet`
    ...
    <Layout navbar={{}}>
      ...
      <div className="col-span-full content-start space-y-4 px-2 pt-4 md:col-span-4 lg:col-span-5 lg:col-start-2 lg:px-0 2xl:col-span-4 2xl:col-start-3 3xl:col-start-5 4xl:col-start-7">
        ...
        <div className="hidden space-y-4 md:block">
          <TokenInfo token={token.token} hotpotListing={hotpotListings.data} />
        </div>
      </div>
...
export const getStaticProps: GetStaticProps<{
  ...
  const listingsUrl = new URL(
    `${HOTPOT_API}/order/collection/${contract}/token/${tokenId}?chain=mainnet`
  )
  ...
  return {
    props: {
      collectionId,
      tokenDetails: data?.tokens?.[0]?.token,
      hotpotListing,
    },
  }
}
