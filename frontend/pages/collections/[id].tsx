...
const Home: NextPage<Props> = ({ fallback, id }) => {
  ...
  const hotpotListingsUrl = `${HOTPOT_API}/order/collection/${id}?chain=mainnet`
...
export const getStaticProps: GetStaticProps<{
  ...
  const listingsUrl = new URL(
    `${HOTPOT_API}/order/collection/${id}?chain=mainnet`
  )
  const listingsRes = await fetch(listingsUrl.href)
  const listings = await listingsRes.json()

  return {
    props: { fallback: { collection, tokens, attributes, listings }, id },
    revalidate: 20,
  }
}
